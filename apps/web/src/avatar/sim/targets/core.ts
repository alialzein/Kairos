...
/** Dense amber core: 60 % in the head, 40 % in the chest anchor (docs/06 §2 CORE). The head
 *  cluster sits at `coreHead` (crown) rather than `head` (eye level) so its bloom stops
 *  eclipsing the eyes on the bust. */
export function core(n: number, rng: Rng): Float32Array {
  const out = new Float32Array(n * 3);
  const headCount = Math.round(n * 0.6);
  for (let i = 0; i < n; i++) {
    const head = i < headCount;
    const anchor = head ? ANCHORS.coreHead : ANCHORS.chest;
