...
describe("ringSpecs", () => {
  it("steps outward from innerRadius and fades from opacityFrom to opacityTo", () => {
    ...
    expect(r[0]).toEqual({ radius: 0.9, opacity: 0.5 });
    expect(r[8]?.radius).toBeCloseTo(0.9 + 8 * 0.25, 9);
    expect(r[8]?.opacity).toBeCloseTo(0.06, 9);
  ...
  it("handles a single ring and zero rings", () => {
    expect(ringSpecs({ ...sceneConfig.rings, count: 1 })).toEqual([{ radius: 0.9, opacity: 0.5 }]);
