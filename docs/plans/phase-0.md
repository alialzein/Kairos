...
- [ ] **Step 3b (edge hardening, from the Phase 0 final review):** disable FastAPI docs in production (`docs_url=None, redoc_url=None, openapi_url=None` unless `EXPOSE_DOCS=true`), keep `/brain/metrics` off the public Caddyfile (`@metrics path /brain/metrics` → `respond 404`; scrape over the Docker network), add a non-root `USER` to every service Dockerfile, and pin base images by digest.
