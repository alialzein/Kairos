...
| **Bench page** | `/bench/avatar`: a public page that renders only the Avatar canvas, driven by query parameters (`tier`, `state`, `demo`, `webgl` — CI passes `webgl=1` because the runner has no working WebGPU), used by the perf and E2E tests. Shows no persona or user data. |
...
| **Neural Bust scene** | Avatar look v3 (`docs/plans/scene-plan.md`, log in `docs/plans/scene-log.md`): a standalone R3F scene under `apps/web/src/scene/` — opaque contour-line bust, face core, neck circuitry, rings, landscape, bloom, HUD — every value in `sceneConfig.ts`, every layer behind a flag. A static LISTENING look; not yet wired to the Avatar States. |
| **Scene bench** | `/bench/scene`: public page rendering only the Neural Bust scene, driven by `phase` / `only` / `off` / `webgl` / `set` query parameters (`set` applies `sceneConfig` overrides for one page load). Used by the e2e smoke and the phase screenshots in `docs/screens/`. |
